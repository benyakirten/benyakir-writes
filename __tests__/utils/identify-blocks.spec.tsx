...
const TestBlock1: React.FC<{ title?: string, lang?: string, type: string }> = ({ title, lang }) => (
    <div lang={lang} title={title} />
)
const TestBlock2: React.FC<{ title?: string, lang?: string , type: string}> = ({ title, lang }) => (
    <div lang={lang} title={title} />
)

const TestBlock3: React.FC<{ title?: string, lang?: string, type: string }> = ({ title, lang }) => (
    <div lang={lang} title={title} />
)

const TestBlock4: React.FC<{ title?: string, lang?: string, type: string }> = ({ title, lang }) => (
    <div lang={lang} title={title} />
)
...
describe("identify-blocks util", () => {
    describe("findBlock", () => {
        ...
        it("should throw errors for incorrectly formatted inputs", () => {
            ...
            for (let input of badInputs) {
                expect(() => idBlocks.findBlock(input.block, input.blockClass)).toThrow()
            }
        });
        ...
        it("should not create an empty block if the last element is a custom block", () => {
            ...
            expect(idBlocks.findBlock(block, blockClass)).not.toEqual(incorrectResult)
        });
    });
    ...
    describe("processWPEntry", () => {
        it('should process multiple separate blocks without creating intermediate empty blocks', () => {
            const expectations: { content: string, result: any[] }[] = [
                {
                    content: '<p>some content goes here</p><div class="test-block-class-1"><pre style=\"display: none;\">{\"title\":\"test\"}</pre></div><p>continued content here</p><div class="test-block-class-2"><pre style=\"display: none;\">{\"lang\":\"en\"}</pre></div><p>end content</p>',
                    result: [
                        {
                            type: 'default',
                            content: '<p>some content goes here</p>'
            ...
            for (let expectation of expectations) {
                expect(idBlocks.preprocessWPEntry(expectation.content)).toEqual(expectation.result)
            }
        })
    });
    ...
    describe("createBlocks", () => {
        it('should return a simple div if the input has length 0', () => {
            expect(idBlocks.createBlocks([])).toEqual([<div>Nothing to see here</div>])
        })
        ...
        it('should return known results for known inputs', () => {
            const expectations: { content: any[], results: JSX.Element[] }[] = [
                {
                    ...
                    results: [
                        <WpContent dangerouslySetInnerHTML={{ __html: '<p>some content goes here</p>' }} />,
                        <TestBlock1 title="test" type="test-block-class-1" />,
                        <WpContent dangerouslySetInnerHTML={{ __html: '<p>continued content here</p>' }} />,
                        <TestBlock2 lang="en" type="test-block-class-2" />,
                        <WpContent dangerouslySetInnerHTML={{ __html: '<p>end content</p>' }} />,
            ...
            for (let expectation of expectations) {
                expect(idBlocks.createBlocks(expectation.content)).toEqual(expectation.results)
            }
        })
