...
import { BlogPostType, FlattenedBlogPost } from '@Types/posts'
...
describe('blog util', () => {
  ...
  describe('createMetaForBlogPost', () => {
    it('should produce known results for known inputs', () => {
      ...
      expect(resultOne.slice(5, 9).join(" ")).toEqual(postOne.title.toLowerCase())
      ...
      expect(resultTwo.slice(0, 2).join(" ")).toEqual(postTwo.tags![1].toLowerCase())
      expect(resultTwo.slice(2, 4).join(" ")).toEqual(postTwo.tags![0].toLowerCase())
      expect(resultTwo.slice(4, 6).join(" ")).toEqual(postTwo.categories![1].toLowerCase())
      expect(resultTwo.slice(6, 8).join(" ")).toEqual(postTwo.categories![0].toLowerCase())
      ...
      expect(resultTwo.slice(12, 16).join(" ")).toEqual(postTwo.content?.toLowerCase())
      expect(resultTwo[16]).toEqual(postTwo.slug.toLowerCase())
      expect(resultTwo.slice(17, 21).join(" ")).toEqual(postTwo.title.toLowerCase())
    })
  })
  ...
  describe('formatBlogPost', () => {
    it('should process the information in a particular way', () => {
      ...
      expect(flattenedPostOne.published).toEqual(getBlogPostDateInformation(postOne.date))
      ...
      expect(flattenedPostTwo.published).toEqual(getBlogPostDateInformation(postTwo.date))
      expect(flattenedPostTwo.categories).toEqual([
        'cat A',
        'cat B'
      ])
      expect(flattenedPostTwo.tags).toEqual([
        'tag A',
        'tag B'
      ])
    })
