...
import { copyThemeByID, createTheme, deleteThemeByID, reorderThemes, resetThemeOptions, setActiveThemeByID, setThemePreferenceByID, toggleUseComputerPreferences } from "@Store/theme/theme.slice";
...
describe('theme page', () => {
  ...
  it('should render two lists, one with the day and night theme and one with the four options', async () => {
    ...
    expect(lists[1].children[0].textContent?.includes("Preferred Theme")).toEqual(true)
    expect(lists[1].children[1].textContent?.includes("Active Theme")).toEqual(true)
    expect(lists[1].children[2].textContent?.includes("Copy Theme")).toEqual(true)
    expect(lists[1].children[3].textContent?.includes("Delete Theme")).toEqual(true)
  })
  ...
  it('should render the elements under Modify Theme if a theme is clicked', async () => {
    ...
    const propertiesList = textBox.parentElement!.parentElement!.parentElement!.children[2]
    expect(propertiesList.children.length).toEqual(Object.keys(defaultDayTheme).length - 2)
  })
  ...
  it('should call dispatch the reorderThemes action if the first list items are dragged and dropped appropriately', async () => {
    ...
    expect(mockDispatch).toHaveBeenCalledTimes(2)
    expect(mockDispatch).toHaveBeenCalledWith(reorderThemes({
      start: "",
      end: "0",
  ...
  it('should render four buttons for every theme that calls the appropriate actions - and the delete button cannot be activated', async () => {
    ...
    const buttons = themeList.children[0].firstElementChild!.children[1].children
  ...
  it('should dispatch the appropriate action if an item is dropped onto the second list', async () => {
    ...
    expect(mockDispatch).toHaveBeenCalledTimes(2)
    expect(mockDispatch).toHaveBeenCalledWith(setThemePreferenceByID('test-value'))
