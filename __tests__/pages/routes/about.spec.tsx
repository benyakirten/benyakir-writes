...
import type { SVGImageQuery } from "@Types/query";
...
describe("about page", () => {
		...
		data: {
			allFile: {
				...
				nodes: allIcons.filter(
					(i) =>
						i.name === "Twitter" || i.name === "GitHub" || i.name === "Blog",
				),
			},
		},
	};
	...
	it("should render a heading with the page title with a following paragraph", async () => {
		...
		expect(para.firstElementChild?.getAttribute("href")).toEqual(
			"https://benyakiredits.com",
		);
	});
