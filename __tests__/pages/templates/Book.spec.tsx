...
describe('book template', () => {
      ...
      data: {
        wpBook: {
          ...
          book: {
            ...
            coverDesignerLinks: 'https://bio-a.com, https://bio-b.com',
  ...
  describe('the cover', () => {
    ...
    it('should render a fallback image if the book has no cover', async () => {
      ...
      expect(coverTwo.nextElementSibling?.textContent).toEqual('Test Book B')
  ...
  it("should render a list with the cover designer's name and info underneath the date if they are present", async () => {
    ...
    const coverDesignerLinks = [parentList.children[3], parentList.children[4]]
    expect(coverDesignerLinks[0].firstChild?.firstChild?.textContent).toEqual(
      'Bio A'
    )
    expect(
    ...
    expect(coverDesignerLinks[1].firstChild?.firstChild?.textContent).toEqual(
      'Bio B'
    )
    expect(
  ...
  it('should render another list following the cover designer if there is a project with a link and its details', async () => {
    render(<Book data={testData[0].data} />)
    const projectLink = await screen.getByText(
