...
describe("post template", () => {
  ...
  const secondPost: WpPost = {
    data: {
      wpPost: {
        ...
        content:
          "<p>this is a blog post B, yo</p><p>this is a second blog post for B, yo</p>",
        categories: {
          nodes: [
            {
              name: "cat A",
  ...
  it("should render a subheading that says posted with the post date in a specific format afterward", async () => {
    ...
    expect(siblingOne).toBeDefined();
    expect(siblingOne?.textContent).toEqual(
      new Date(firstPost.data.wpPost.date).toLocaleString("en-US", {
        year: "numeric",
        month: "long",
        day: "2-digit",
      })
    ...
    expect(siblingTwo).toBeDefined();
    expect(siblingTwo?.textContent).toEqual(
      new Date(secondPost.data.wpPost.date).toLocaleString("en-US", {
        year: "numeric",
        month: "long",
        day: "2-digit",
      })
    ...
    expect(siblingThree?.textContent).toEqual(
      new Date(thirdPost.data.wpPost.date).toLocaleString("en-US", {
        year: "numeric",
        month: "long",
        day: "2-digit",
      })
  ...
  it("should render a section containing the identified blocks", async () => {
    ...
    expect(contentTwo.nextElementSibling?.textContent).toEqual(
      "this is a second blog post for B, yo"
    );
