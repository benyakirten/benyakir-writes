...
const DraggableThemeList: React.FC<DraggableThemeListProps> = ({ open, onSelect, selectedTheme }) => {
  ...
  const themeNames = React.useMemo(() =>
    themeStore.themes.map((theme) => {
      ...
      return {
        dragValue: theme.id,
        value: (<DraggableTheme nodes={nodes} themeId={theme.id} themeName={theme.name} open={open} />)
      };
    }), [themeStore, open]);
