...
export function flattenTheme(theme: BaseTheme): ThemeGroups {
  ...
  const _theme = Object.keys(theme)
    ...
    .reduce((acc, next) => ({ ...acc, [next]: theme[next as keyof BaseTheme] }), {})
  function recursiveFlattenControls(controlGroup: RecursiveControlGroup, accessors: ThemeAccessors): ThemeGroup {
    let controls: string[][] = []
  ...
  for (let key in _theme) {
    const flattenedGroup = recursiveFlattenControls(_theme[key as keyof typeof _theme], [key])
