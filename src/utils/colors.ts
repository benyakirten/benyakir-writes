...
export function convertHexToRGBA(color: string, opacity: number = 0.4) {
  const parsedColors = convertHexToRGBNumber(color);
  return `rgba(${parsedColors.red}, ${parsedColors.green}, ${parsedColors.blue}, ${+opacity.toFixed(2)})`
}

function parseAndCheck(..._colors: string[]) {
  const parsedColors = _colors.map(c => parseInt(c, 16));
  for (let parsedColor of parsedColors) {
    if (
      isNaN(parsedColor) ||
      parsedColor < 0 ||
      parsedColor > 255
...
function checkIfHexValid(color: string) {
  if (color[0] !== '#' || !(color.length === 4 || color.length === 7)) {
    throw new Error('Hex string must consist of # followed by three or six numbers')
  }
}
...
export function changeHex(color: string, percent: number, positive: boolean = true): string {
  checkIfHexValid(color);
