const generatePolygonPoint = (xLimit: number = 100, yLimit: number = 100): [number, number] => {
  const xPos = Math.floor(Math.random() * xLimit)
  const yPos = Math.floor(Math.random() * yLimit)
  return [xPos, yPos]
}

export const generatePolygonPoints = (xLimit: number = 100, yLimit: number = 100, minPoints: number = 6, maxPoints = 12) => {
