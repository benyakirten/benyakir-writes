...
export function findBlock<T extends BaseBlock>(
    ...
    const finalEntries: (DefaultBlock | T)[] = [createDefaultBlock(splitContent[0])];
    for (let potentialBlock of splitContent.slice(1)) {
        ...
        const outsideBlock = potentialBlock.slice(closingIndex + END_DIV_LENGTH).trim();
...
export function preprocessWPEntry(content: string) {
    ...
    for (let BLOCK_CLASS of Object.keys(KNOWN_BLOCK_CLASSES)) {
        for (let i = 0; i < finalResults.length; i++) {
            ...
            if (finalResults[i].type && finalResults[i].type === 'default') {
                const innerResults = findBlock(finalResults[i] as DefaultBlock, BLOCK_CLASS);
...
export function createBlocks<T extends BaseBlock>(blocks: T[]): JSX.Element[] {
    if (blocks.length === 0) return [<div>Nothing to see here</div>];
